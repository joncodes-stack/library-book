...
namespace LibraryBook.CrossCutting.Services
{
    public class EmailService : IEmailService
    {
        ...
        public async Task SendValidatemEmailAsync(string email, string name, int code)
        {
            var emailRequest = new EmailDto();

            var content = BuildConfirmationEmail(name, code);

            emailRequest.Subject = "Validação de Email";
